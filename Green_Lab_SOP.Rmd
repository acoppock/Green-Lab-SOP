...
This is a living document. To suggest changes or additions, please feel free to e-mail us or submit an issue request on GitHub. Also, when referencing this document, please be sure to note the version.
...
# Checks on data preparation and study implementation

We will have at least two team members involved in all raw data collection efforts (including, but not limited to, the gathering of data from survey organizations and government agencies). We will obtain Institutional Review Board approval for these team members to have access to the raw data.

If we use computer programs to construct variables for analysis, we will make these programs publicly available.
...
- Verification that treatment occurred. Supportive evidence can include receipts for expenses associated with treatment, spot checks by more than one observer in the field, geotagged photographs, and/or manipulation checks (statistical evidence that random assignment created a treatment contrast in the intended direction—see, e.g., @Gerber2012, Box 10.3, p. 335).

- Verification that outcome data were gathered (e.g., via receipts, spot checks, and/or having a second team member independently gather data for a sample of cases).

- Examination of the distributions and missingness rates of all variables used in the analysis and their precursors in the raw data files. These variables should be checked both for plausibility and for consistency across the stages of data processing. The checks should normally include not only univariate summaries, but also cross-tabulations of related variables.
...
We will report two-tailed hypothesis tests unless the PAP specifies a one-tailed test or some other approach.^[@Olken2015 discusses one other approach: "An interesting hybrid alternative would be to pre-specify asymmetric tests – i.e. to reject the null if the result was
...
## Example: Studentized Permuation Test
...
# Obtain Heteroskedasticty-robust t-statisic
t_obs <- coeftest(fit, vcov = vcovHC(fit, type = "HC2"))[2,3]
...
for(i in 1:sims){
  ...
  t_sims[i] <- coeftest(fit_sim, vcov = vcovHC(fit_sim, type = "HC2"))[2,3]
}
...
We will perform a statistical test to judge whether observed covariate imbalances are larger than would normally be expected from chance alone. In an experiment with a binary treatment and a constant probability of assignment to treatment, the test involves a regression of the treatment indicator on the covariates and calculation of the F-statistic for the hypothesis that all the coefficients on the covariates are zero (@Gerber2012, p. 107). If the experiment is block-randomized with treatment probabilities that vary by block, we will include dummy variables for the varying treatment probabilities in both the restricted and unrestricted regression models used to compute the F-statistic.

We will use a permutation test (randomization inference) to calculate the p-value associated with the F-statistic.

In an experiment with multiple treatments, we will perform an analogous test using multinomial logistic regression of treatment on covariates, calculation of the likelihood-ratio test statistic for the hypothesis that all the coefficients on the covariates are zero, and randomization inference to calculate the p-value.
...
## Example: Permutation Tests of Covariate Balance
```{r, cache=TRUE}
# Generate Covariates
library(randomizr)
...
f_obs <- summary(fit)$f[1]

# Compare to sampling distribution of F
...
1. Rely on second-round sampling of nonrespondents, combined with extreme value bounds,[^4] if (a) the project has adequate resources and (b) it is plausible to assume that potential outcomes are invariant to whether they are observed in the initial sample or the follow-up sample. If either (a) or (b) is not met, go to step 2.
...
In the event that these checks reveal any errors, we will report the errors and take them into account in any analyses we report. We will add more specific guidance and examples to our SOP as we learn from our own and/or other researchers' experiences.
