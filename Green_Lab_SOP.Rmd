...
date: 'Version 0.3f: July 15, 2015'
output:
  pdf_document:
    toc: yes
  html_document:
...
---
\pagebreak
This standard operating procedures (SOP) document describes the default practices of the experimental lab group led by Donald P. Green at Columbia University. These defaults apply to analytic decisions that have not been made explicit in pre-analysis plans (PAPs). They are not meant to override decisions that are laid out in PAPs. For more discussion of the motivations for SOP, see @LinGreen2015.

This is a living document. To suggest changes or additions, please email or submit an issue request on GitHub. Also, when referencing this document, please be sure to note the version.
...
We will use randomization inference (using at least 10,000 randomizations and the random number seed 1234567) to calculate the p-value associated with the F-statistic.
...
We will have at least two team members check each computer program used to randomly assign treatment, and we will make these programs publicly available. In all such programs, we will include a seed value for the random number generator, so that the resulting assignments can be replicated and verified.
...
[to do: describe "external verification that random assignment was implemented as planned"]
...
(As @Morgan2012 [p. 1264] explain, when randomizations with large covariate imbalances are disallowed, traditional methods of analysis that do not take the restriction into account will tend to result in conservative inferences.)
